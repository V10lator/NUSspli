...
static uint8_t otp_vwii_key[16] = { 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00 };
...
static void readKeys()
{
    ...
    if(Mocha_ReadOTP(&otp) == MOCHA_RESULT_SUCCESS)
    {
        ...
        OSBlockMove(otp_vwii_key, otp.wiiUBank.vWiiCommonKey, 16, false);
        ...
        tmp = &ret[0];
        for(int i = 0; i < 16; i++, tmp += 2)
            sprintf(tmp, "%02x", otp_vwii_key[i]);
        debugPrintf("\tvWii common key: %s", ret);
#endif
    }
    else
...
const uint8_t *getVWiiKey()
{
    readKeys();
    return (const uint8_t *)otp_vwii_key;
}
